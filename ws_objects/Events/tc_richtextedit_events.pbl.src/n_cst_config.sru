...
public function integer of_getdbinfo (string as_database);string ls_AutoCommit
...
public function integer of_connect (ref transaction atr_transobject, string as_database);if IsValid(atr_TransObject) then
...
public function string of_getapppath ();is_ApplicationPath = ProfileString(is_Profile, "PATH", "ApplicationPath", "")

Return is_ApplicationPath
end function

public subroutine of_initproperty ();Database = ""
DBMS = ""
ServerName = ""
LogPass = ""
LogID = ""
...
public function integer of_settransproperty (transaction atr_transobject);if Database<>""   then atr_TransObject.Database   = Database
...
public function integer of_connectex (ref transaction atr_transobject, string as_database, string as_flags);as_Flags = Upper(as_Flags)
...
public function boolean of_iscotpbautorun ();string ls_COTPBAutoRun
...
public function integer of_getcotvalidate (transaction atr_cotdb, ref boolean ab_saveresult, ref boolean ab_updateexpvalue);string ls_UserName
string ls_Password
string ls_Upower
string ls_SResult
...
if FileLength(as_text) > 32765 then
	messagebox("提示信息","文本内容过大，创建失败！")
	return false
end if
...
string  ls_name
long    ll_BlobLen,ll_CurrentPos,li_Cnt
